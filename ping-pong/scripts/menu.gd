...
func _on_game_over(win: bool) -> void:
	show()
	if win:
		$results/CenterContainer/VBoxContainer/death_message.text = "You win!"
		$AudioStreamPlayer.stream = load("res://assets/audio/victory_theme.wav")
		$results/CenterContainer/VBoxContainer/image.texture = load("res://assets/ui/win_screen.png")
	else:
		$results/CenterContainer/VBoxContainer/death_message.text = "You lose!"
		$AudioStreamPlayer.stream = load("res://assets/audio/losing_theme.mp3")
		$results/CenterContainer/VBoxContainer/image.texture = load("res://assets/ui/lose_screen.png")
	$AudioStreamPlayer.play()
